...
import { warningFormTheme, FormView } from '../../components/formView';
...
export class ScanProgressPopup extends ComposedPopup {
    ...
    openWith(mode: 'copy' | 'move'): void {
        ...
        view.addComponent(new TextBlock(
            () => [{ text: truncatePath(this.currentPath, textWidth) }], 1));
        view.addSeparator();
        view.addComponent(new TextBlock(() => [
            { text: formatCounter('Folders', longestLabel, this.dirs, 0, false, counterWidth) },
            { text: formatCounter('Files', longestLabel, this.files, 0, false, counterWidth) },
            { text: formatCounter('Bytes', longestLabel, this.bytes, 0, false, counterWidth) },
        ], 3));
...
export class CopyProgressPopup extends ComposedPopup {
    ...
    private currentBar!: ProgressBar;
    private totalBar!: ProgressBar;
    ...
    updateProgress(srcName: string, dstName: string, filesCopied: number,
        ...
        this.currentBar.percent = currentPercent;
        this.totalBar.percent = totalPercent;
    ...
    private buildNormalView(): void {
        ...
        const counterWidth = textWidth - 5;
        this.currentBar = new ProgressBar(CopyProgressPopup.CANVAS_WIDTH, true);
        this.totalBar = new ProgressBar(CopyProgressPopup.CANVAS_WIDTH, true);
        ...
        view.addComponent(new TextBlock(
            () => [{ text: truncatePath(this.state.srcName, textWidth) }], 1));
        view.addLabel('To');
        view.addComponent(new TextBlock(
            () => [{ text: truncatePath(this.state.dstName, textWidth) }], 1));
        view.addComponent(this.currentBar);
        view.addSeparator('Total');
        view.addComponent(new TextBlock(() => [{
            text: formatCounter('Files', 'Bytes',
                this.state.filesCopied, this.state.filesTotal, true, counterWidth),
        }], 1));
        view.addComponent(new TextBlock(() => [{
            text: formatCounter('Bytes', 'Files',
                this.state.bytesCopied, this.state.bytesTotal, true, counterWidth),
        }], 1));
        view.addComponent(this.totalBar);
        view.addSeparator();
        view.addComponent(new TextBlock(() => [{
            text: formatStatsLine(this.startTime, this.state.bytesCopied,
                this.state.bytesTotal, this.getRecentSpeed(), textWidth),
        }], 1));
    ...
    private buildPausedView(): void {
        ...
        view.addComponent(new TextBlock(() => [
            { text: 'Operation has been interrupted' },
            { text: '' },
            { text: 'Currently processing:' },
            { text: truncatePath(this.state.srcName, textWidth), bold: true },
            { text: '' },
...
export class DeleteProgressPopup extends ComposedPopup {
    ...
    openWith(mode: 'file' | 'directory' | 'items', initialPath: string): void {
        ...
        if (showDirs) {
            view.addComponent(new TextBlock(() => [
                { text: formatCounter('Files', longestLabel, this.files, 0, false, counterWidth) },
                { text: formatCounter('Directories', longestLabel, this.dirs, 0, false, counterWidth) },
            ], 2));
        } else {
            view.addComponent(new TextBlock(
                () => [{ text: formatCounter('Files', longestLabel, this.files, 0, false, counterWidth) }], 1));
        }
