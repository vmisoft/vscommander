...
export class CheckboxControl {
    ...
    // Blink the cell inside the brackets between the check state and a cursor
    // bar, so a focused checkbox shows a blinking cursor like an input field.
    renderBlink(row: number, col: number, style: TextStyle, cursorStyle: TextStyle): string {
        this.cursorVisible = !this.cursorVisible;
        const ch = this.cursorVisible ? '|' : (this.checked ? 'x' : ' ');
        const st = this.cursorVisible ? cursorStyle : style;
        return moveTo(row, col) + applyStyle(st) + ch + resetStyle();
    }
