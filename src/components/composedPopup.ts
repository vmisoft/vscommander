...
export abstract class ComposedPopup extends Popup {
    ...
    render(rows: number, cols: number, theme: Theme): string {
        ...
        // Centre on the box itself: when an open dropdown has grown the buffer
        // downward, that extra height must not shift the box upward.
        const anchorHeight = this._activeView
            ? this._activeView.boxHeight + 2 * this.padV
            : fb.height;
        const baseRow = Math.floor((rows - anchorHeight) / 2) + 1;
